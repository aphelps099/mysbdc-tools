...
export default function ZebsMotionStudio() {
  ...
  const syncMedia = useCallback(() => {
    ...
    for (const id in videosRef.current) {
      ...
      const isActive = !!active && active.template === 'video' && active.videoId === id;
      if (isActive) {
        const targetS = (active.videoTrimStart + local) / 1000;
        v.muted = active.videoMuted;
        v.volume = Math.max(0, Math.min(1, active.videoVolume));
  ...
  const handleAudioFile = async (file: File) => {
    ...
    try {
      ...
      setAudioStatus({ ok: true, msg: `${file.name} — ${fmtTime(asset.buffer.duration * 1000)}. Loops under the whole video.` });
    } catch {
      setAudioStatus({ ok: false, msg: 'Could not decode that file — MP3, WAV, or M4A work best.' });
    }
  };
