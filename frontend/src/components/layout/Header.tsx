...
export default function Header({ onMenuToggle, onOpenAbout }: HeaderProps) {
  return (
    <header className="md:hidden fixed top-0 left-0 right-0 z-40 h-[var(--header-height)] bg-[var(--navy)] flex items-center justify-between px-4">
      ...
      <button
        onClick={onOpenAbout}
        className="text-[11px] font-[var(--mono)] tracking-[0.08em] uppercase text-white/40 hover:text-white/70 underline underline-offset-2 decoration-white/20 transition-colors duration-[var(--duration-fast)] cursor-pointer"
        aria-label="About"
      >
        about
      </button>
