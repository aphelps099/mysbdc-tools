...
describe('derived metrics (seed data, dashboard numbers)', () => {
  it('matches the metric strip in the design reference', () => {
    const m = computeMetrics(SEED_PARTNERS, TODAY);
  ...
  it('lists overdue partners before going-stale ones, never dormant', () => {
    const items = attentionItems(SEED_PARTNERS, TODAY);
  ...
  it('flags a non-dormant partner as going stale after 45 days', () => {
    const partners = SEED_PARTNERS.map((p) =>
...
describe('dashboard bars', () => {
  it('builds one stage bar per stage in pipeline order', () => {
    const bars = stageBars(SEED_PARTNERS);
...
describe('table filtering and sorting', () => {
  it('AND-combines filters and searches across name/contact/city/subtype/center', () => {
    expect(filterPartners(SEED_PARTNERS, { q: 'eureka', fType: '', fStage: '', fOwner: '' })).toHaveLength(2);
    expect(filterPartners(SEED_PARTNERS, { q: 'eureka', fType: 'Referral', fStage: '', fOwner: '' })).toHaveLength(1);
    expect(filterPartners(SEED_PARTNERS, { q: '', fType: '', fStage: 'Active', fOwner: 'Aaron' })).toHaveLength(3);
    expect(filterPartners(SEED_PARTNERS, { q: 'dana', fType: '', fStage: '', fOwner: '' })).toHaveLength(1);
  });

  it('sorts stage by pipeline order, not alphabetically', () => {
    const sorted = sortPartners(SEED_PARTNERS, 'stage', 1);
  ...
  it('flips with sortDir and sorts referrals numerically', () => {
    const desc = sortPartners(SEED_PARTNERS, 'referrals', -1);
...
describe('activity feed', () => {
  it('flattens all activities date-desc and filters by type', () => {
    const feed = activityFeed(SEED_PARTNERS, '');
    expect(feed).toHaveLength(SEED_PARTNERS.reduce((s, p) => s + p.activities.length, 0));
    for (let i = 1; i < feed.length; i++) {
      expect(feed[i - 1].date >= feed[i].date).toBe(true);
    }
    const referrals = activityFeed(SEED_PARTNERS, 'Referral');
...
describe('nextPartnerId', () => {
  it('is one more than the max id (seed ids are non-contiguous)', () => {
    expect(nextPartnerId(SEED_PARTNERS)).toBe(19);
